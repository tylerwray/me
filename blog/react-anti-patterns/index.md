...
to the `jsx+<Inner />` component when they don't need to! And what happens when some of those props become optional? When the `jsx+<Inner />`
component wants to rename one of those props?

Also notice how the `jsx+<Outer />` component doesn't actually use any of the props, it just passes them along!

This approach couples the two components together, you may as well inline the `jsx+<Inner />` component inside the `jsx+<Outer />`
